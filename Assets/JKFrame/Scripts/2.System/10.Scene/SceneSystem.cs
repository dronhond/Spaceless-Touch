...
namespace JKFrame
{
    public static class SceneSystem
    {
        ...
        private static IEnumerator DoLoadSceneAsync(string sceneName, Action<float> callBack = null, LoadSceneMode mode = LoadSceneMode.Single)
        {
            ...
            while (progress < 1)
            {
                ...
                if (progress != ao.progress)
                {
                    ...
                    // 把加载进度分发到事件中心
                    JKEventSystem.EventTrigger("LoadingSceneProgress", ao.progress);
                    if (progress == 1)
                    {
                        JKEventSystem.EventTrigger("LoadSceneSucceed");
                        break;
                    }
                }
        ...
        public static void LoadSceneAsyncByPath(string scenePath, Action callBack = null, LoadSceneMode mode = LoadSceneMode.Single)
        {
            var handle = Addressables.LoadSceneAsync(scenePath, mode);
            MonoSystem.Start_Coroutine(DoLoadSceneAsync(handle, callBack));
        }

        private static IEnumerator DoLoadSceneAsync(AsyncOperationHandle<SceneInstance> handle, Action callBack)
        {
            while (handle.PercentComplete < 1)
            {
                yield return CoroutineTool.WaitForFrames();
                if (handle is not { PercentComplete: 1 }) continue;
                callBack?.Invoke();
                break;
            }
        }
